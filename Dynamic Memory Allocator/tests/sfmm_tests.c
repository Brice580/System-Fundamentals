...
Test(sfmm_student_suite, coalesce_prev_and_next_test, .timeout = TEST_TIMEOUT){
	
	sf_errno = 0;
	char* x  = sf_malloc(1000);
 
	char* y = sf_malloc(2000); //alloc bloc

	char* z = sf_malloc(1500);

	sf_free(x); //free block in heap
	sf_free(z);

	sf_free(y); //coalesce with both before and after
	
	assert_free_list_size(8,1);
	assert_free_block_count(8152, 1); //combined of before and after

	cr_assert(sf_errno == 0, "Errno set incorrectly!");



	
}
Test(sfmm_student_suite, free_flush_test, .timeout = TEST_TIMEOUT){
	sf_errno = 0;
	...
	assert_quick_list_block_count(56,5);

	sf_free(y);

	cr_assert(y != NULL, "Malloc returned NULL!");
	assert_quick_list_block_count(56,1);
	
	assert_free_block_count(280,1 );
	cr_assert(sf_errno == 0, "Errno not set properly!");
...
Test(sfmm_student_suite, invalid_memalign_pointer_test, .timeout = TEST_TIMEOUT){

	char* a = sf_memalign(25, 3);

	cr_assert(sf_errno == EINVAL, "Errno not set properly!");
	cr_assert(a == NULL, "Expected to return NULL!");

	char* b = sf_memalign(50, 17);

	cr_assert(sf_errno == EINVAL, "Errno not set properly!");
	cr_assert(b == NULL, "Expected to return NULL!");

}

Test(sfmm_student_suite, free_alloc_bit_test, .timeout = TEST_TIMEOUT){
	sf_errno = 0;
	char* p = sf_malloc(1234);
	char* c = sf_malloc(4500);

	sf_free(p);
	sf_free(c);

	sf_block* blx = (sf_block *)((char *)p - sizeof(sf_header));
	sf_block* xyz = (sf_block *)((char *)c - sizeof(sf_header));

	cr_assert((blx->header & THIS_BLOCK_ALLOCATED) == 0, "Alloc bit is not set properly!");
	cr_assert((xyz->header & THIS_BLOCK_ALLOCATED) == 0, "Alloc bit is not set properly!");
	
	cr_assert(sf_errno == 0, "Errno is not set correctly");


}

Test(sfmm_student_suite, realloc_free_blocks, .timeout = TEST_TIMEOUT){
	sf_errno = 0;
	char* p = sf_malloc(1234);

	char* re_p = sf_realloc(p, 4053);
	...
	assert_quick_list_block_count(56, 1);
	assert_free_block_count(1192, 1);
	assert_free_block_count(2840, 1);

	sf_block* block = (sf_block *)((char *)re_p - sizeof(sf_header));
	
	cr_assert((block->header & ~0x7) == 4064, "Realloc'ed block size not what was expected!");
